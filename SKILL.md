...
- **当前仓库已上传图片**: 小米 5 项、vivo 6 项（含 SleepMode）、华为/荣耀系 6 项、纯鸿蒙 4 项、iOS 4 项、OPPO 6 项（含 AppFreeze）全覆盖。完整文件清单见第二步「图片可用性」
...
> 当前仓库图片分布：小米 5 项、vivo 6 项（含 SleepMode）、华为/荣耀系 6 项、纯鸿蒙 4 项、iOS 4 项、OPPO 6 项（含 AppFreeze）图片全部齐全。若后续新增操作无图，按异常处理返回提示，不要编造链接。
...
1. 按品牌（及系统）从第一步映射表查得品牌文件前缀，按 `{品牌文件前缀}-{operation_name}{可选-序号}.jpg` 拼接完整 URL（前缀 `https://cdn.jsdelivr.net/gh/linknowaaa/ICS@main/` + 文件名）。**拼接前先对照第二步「图片可用性」确认该品牌该操作是否有图，无图不拼接**：
   - 华为（鸿蒙）4 项：`https://cdn.jsdelivr.net/gh/linknowaaa/ICS@main/HarmonyOS-LocationPermission.jpg`（DeviceDiscovery / PowerSaving / BackgroundLock 同理）
