...
- **图片链接前缀（IMAGE\_BASE）**: `https://cdn.jsdelivr.net/gh/linknowaaa/ICS@main/`
...
>
> **弹出方式（按当前环境能力执行，优先级从高到低，命中即止）**：
>
> 1. **环境提供交互式提问/选择工具时（TRAE 桌面端 AskUserQuestion、WorkBuddy 移动端选择卡片等）——必须调用该工具，在回答过程中弹出选择卡片让用户点选**。系统未确认时一次弹出两页（第 1 页「系统」、第 2 页「操作类型」，即卡片分页 1/2、2/2）；系统已确认时只弹「操作类型」一页。弹出后**立即停止输出、等待用户点选**，禁止边弹卡片边把指引先输出完，禁止用纯文字提问代替可用的交互卡片。
> 2. **环境确实无任何交互提问能力（纯文本）时**：改用**编号文字选项**并停下等回复。统一输出格式（系统已确认时只列该系统支持的操作，并附「全部都要」「我不清楚」）：
>
> 「请选择需要哪一项的操作指引：① 基础权限　② 自启动　③ 后台运行/耗电　④ 后台锁定　⑤ 省流量　⑥ 全部都要　⑦ 我不清楚（也可先描述现象帮你排查）→ 请回复序号或关键词。」
>
> **选择卡片规格（与线上样式一致）**：
>
> - **第 1 页「系统」**：小标题 = 「{品牌}系统」（如「华为系统」）；问题 = 「你的{品牌}手机是什么系统？」；选项按该系统族候选编号列出（如华为：① 纯鸿蒙 5.0及以上　② 鸿蒙4.2及以下/EMUI/荣耀　③ 我不清楚），并保留「或输入其他选项」自由输入。
>
> - **第 2 页「操作类型」**：小标题 = 「操作类型」；问题 = 「你需要{品牌}手机的哪一项操作指引？」；选项 = 该系统支持的操作按编号列出（见下方②），末尾附「全部都要」「我不清楚」，并保留自由输入。
>
> - **弹出时机**：允许在弹卡片前先输出简短说明（如"该品牌涵盖多套指引，需先区分系统"之类的关键提示），但说明必须简短，且弹卡片后必须停下等选择，用户选定前不得输出任何指引图片或步骤。
>
...
当出现以下任一情况时，**必须弹出选择卡片**（环境无交互能力时才退化为编号文字选项），**不得直接跳过、不得默认猜测后直接给出**，用户选定后再继续回答：
...
1. 常见型号前缀线索：`M20xx / Mi / MIX / K系列 / Note系列(Redmi)` → 小米系；`V19xx / V20xx / X系列 / S系列 / iQOO` → vivo 系；`ALN / NOH / BKL / HRY / ANY / Mate / P系列 / 荣耀Magic系列` → 华为系（EMUI / MagicOS）。
...
| 用户问题关键词                              | 操作名（operation\_name） | 是否有图片                                                  |
| ------------------------------------ | -------------------- | ------------------------------------------------------ |
...
| 品牌   | 适用机型                             | 用户问题关键词               | 操作名（operation\_name） | 是否有图片                     |
| ---- | -------------------------------- | --------------------- | -------------------- | ------------------------- |
...
1. 回复模板（图片 + 文字操作指引一起返回；图片一律按下方「图片输出策略」输出）：
