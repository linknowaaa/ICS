...
本技能支持两种环境的自我更新：可读写本地文件的环境（如 TRAE，写入文件持久化同步），以及 WorkBuddy、Kimi 等移动端纯提示词环境（本次对话直接按远程最新版执行，无需写入文件）。

1. **获取远程最新内容**：优先 GitHub raw `https://raw.githubusercontent.com/linknowaaa/ICS/main/SKILL.md`；若无法访问（国内网络常见），改用 jsDelivr `https://cdn.jsdelivr.net/gh/linknowaaa/ICS@main/SKILL.md`（可能有缓存延迟，若怀疑拿到旧内容，可用 `https://purge.jsdelivr.net/gh/linknowaaa/ICS@main/SKILL.md` 刷新缓存后重试）。
