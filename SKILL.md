...
description: "Identifies phone brand (Xiaomi/vivo/Honor etc.) from user screenshots or text, returns GitHub/jsDelivr-hosted guide images, and answers offline/no-data troubleshooting questions. Invoke when user uploads phone screenshots, asks phone operation guides, or reports offline/no-data issues."
---
...
| HarmonyOS（鸿蒙） | 华为 | HarmonyOS | 鸿蒙：LocationPermission、DeviceDiscovery、PowerSaving、BackgroundLock |
...
| iOS | 苹果 | 暂未上传 | iOS：LocationPermission、MotionFitness、BackgroundRefresh、LowPowerMode |
...
| huawei、HUAWEI、华为、honor、HONOR、荣耀、EMUI、MagicOS、Magic UI、鸿蒙 | 华为系 | HarmonyOS 或 Huawei，见下方华为分系统规则 | 鸿蒙4项 或 华为系6项，见下方华为分系统规则 |
...
| apple、iphone、苹果、iOS | 苹果 | 暂未上传 | iOS：LocationPermission、MotionFitness、BackgroundRefresh、LowPowerMode |

- 若能识别出具体型号（如「小米 14 Pro」「vivo X100」），记录型号用于回复展示；但指引图片按品牌粒度返回，不区分型号。
- 华为分系统规则：识别到「鸿蒙 / HarmonyOS」→ 品牌文件前缀 `HarmonyOS`、操作类型为鸿蒙4项（LocationPermission、DeviceDiscovery、PowerSaving、BackgroundLock）；识别到「EMUI / MagicOS / Magic UI / 荣耀 / 安卓」→ 品牌文件前缀 `Huawei`、操作类型为华为系6项（BasicPermissions、Autostart、Background、BatteryWhitelist、BackgroundLock、DataSaver）；两者都出现或不确定时，向用户确认具体系统。
...
|---|---|---|
| 权限、授权、允许、权限设置、应用权限、授予权限、权限管理 | BasicPermissions | ✅ 有 |
| 自启动、自启、后台启动、开机自启、允许自启动、启动管理、自启动管理 | Autostart | ❌ 暂未上传 |
| 后台运行、保活、省电、电池优化、后台、锁屏运行、关闭后台、后台耗电 | Background | ❌ 暂未上传 |
| 后台锁定、锁后台、后台应用锁定、任务锁定、应用锁、后台保留、不被清理 | BackgroundLock | ❌ 暂未上传 |
| 省流量、流量节省、数据节省、节省流量、智能省流量、流量保护 | DataSaver | ❌ 暂未上传 |
...
| 权限、授权、允许、权限设置、应用权限、授予权限、权限管理 | BasicPermissions | ✅ 有 |
| 自启动、自启、后台启动、开机自启、允许自启动、启动管理、自启动管理 | Autostart | ❌ 暂未上传 |
| 后台运行、保活、省电、后台、锁屏运行、关闭后台、后台耗电（不含电池优化） | Background | ❌ 暂未上传 |
| 电池白名单、白名单、后台白名单、耗电白名单、电池优化白名单 | BatteryWhitelist | ❌ 暂未上传 |
| 后台锁定、锁后台、后台应用锁定、任务锁定、应用锁、后台保留、不被清理 | BackgroundLock | ❌ 暂未上传 |
| 省流量、流量节省、数据节省、节省流量、智能省流量、流量保护 | DataSaver | ❌ 暂未上传 |
...
| 用户问题关键词 | 操作名（operation_name） | 是否有图片 |
|---|---|---|
| 位置权限、定位、定位权限、位置服务、位置信息、GPS、定位服务 | LocationPermission | ❌ 暂未上传 |
| 设备发现和连接权限、设备发现、附近设备、发现设备、设备互联、蓝牙连接、超级终端、多设备协同 | DeviceDiscovery | ❌ 暂未上传 |
| 省电模式、省电、超级省电、省电开关、续航、电池省电 | PowerSaving | ❌ 暂未上传 |
| 后台锁定、锁后台、后台应用锁定、任务锁定、应用锁、后台保留、不被清理 | BackgroundLock | ❌ 暂未上传 |
...
|---|---|---|
| 位置权限、定位、定位权限、位置服务、位置信息、GPS、定位服务 | LocationPermission | ❌ 暂未上传 |
| 运动与健身、运动、健身、健康、运动数据、健康数据、运动权限 | MotionFitness | ❌ 暂未上传 |
| 后台App刷新、后台刷新、后台应用刷新、App刷新 | BackgroundRefresh | ❌ 暂未上传 |
| 低电量模式、低电量、省电模式、省电 | LowPowerMode | ❌ 暂未上传 |
...
|---|---|---|---|---|
| vivo | 老版本手机（Funtouch OS / 老版 OriginOS） | 睡眠模式、睡眠、关闭睡眠模式 | SleepMode | ❌ 暂未上传 |
| OPPO | 老机型（老版 ColorOS） | 应用速冻、关闭应用速冻、应用速冻设置、速冻 | AppFreeze | ❌ 暂未上传 |
...
> 当前仓库仅上传了「权限 / 授权 / 允许」对应的 `BasicPermissions` 图片（通用安卓品牌）。若用户询问其他操作，按异常处理返回提示，不要编造链接。

### 第三步：拼接链接并返回

1. 按品牌（及系统）从第一步映射表查得品牌文件前缀，按 `{品牌文件前缀}-{operation_name}.jpg` 拼接完整 URL（前缀 `https://cdn.jsdelivr.net/gh/linknowaaa/ICS@main/` + 文件名）：
...
按「可能原因」逐条列出；原因带关联操作的，结合品牌拼接对应操作指引图（仅当该品牌支持该操作且图片已上传时附图，当前仅 BasicPermissions 有图），其余注明指引图暂未上传。
...
| 权限设置正常但离线 | 1.区域网络环境差；2.在地下室/电梯里；网络正常后数据会补传（无关联操作） |
...
- **品牌无图片**：苹果的权限指引图片尚未上传到仓库，明确告知用户图片暂缺，不要编造链接。
- **操作无图片**：除「权限 / 授权 / 允许」（BasicPermissions）外，其余操作（自启动、后台锁定、省流量、电池白名单、睡眠模式、应用速冻、位置权限等）均暂无图片，明确告知用户该操作指引尚未上传。
- **系统不支持该操作**：鸿蒙系统仅支持位置权限、设备发现和连接权限、省电模式、后台锁定；iOS 仅支持位置权限、运动与健身、后台App刷新、低电量模式。用户询问该操作系统不支持的操作时，明确告知不支持，不要编造图片链接。
